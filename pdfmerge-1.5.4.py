...
class PDFMergerUI:
    def __init__(self, root):
        ...
        # 使用者自訂頁面
        range_frame = Frame(self.frame, pady=5)
        range_frame.pack(pady=5)
        
        self.combobox = Combobox(range_frame, values=["奇數頁", "偶數頁", "倒數第○頁", "最後一頁"], state="readonly")
        self.combobox.grid(row=0, column=0, padx=10)
        self.range_button = Button(range_frame, text="一鍵設定", font=("Arial", 12), width=10, command=lambda: self.set_customized_page(self.combobox.get()))
        self.range_button.grid(row=0, column=1, padx=5)

        # 設為全部頁面的按鈕
        self.set_all_button = Button(self.frame, text="📚 一鍵設為全部", font=("Arial", 12), width=25, command=self.set_all_to_all_pages)
